...
public final class ArraySiftDownMain {
    ...
    private static void siftDown(int[] array, int top, int last) {
        ...
        int Node = array[top];
        int leftNode = array[2*top+1];
        int rightNode = array[2*top+2];
        if(Node > leftNode && Node < rightNode) {
        	exchangeEntries(array, 2*top+1, last);
        	siftDown(array, 2*top+1, last);
        }
        else if(Node < leftNode && Node > rightNode) {
        	exchangeEntries(array, 2*top+1, last);
        	siftDown(array, 2*top+2, last);
        }
        else {
        	int addd;
        	...
        	exchangeEntries(array, 2*top+addd, last);
        	siftDown(array, 2*top+addd, last);
        }
